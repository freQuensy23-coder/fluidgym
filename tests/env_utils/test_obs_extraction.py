    ...
    for agent_idx in range(n_agents):
        ...
        if start < 0:
            expected_window = torch.cat(
                (field_2d[:, start % (n_agents * agent_width) :], field_2d[:, :end]),
                dim=1,
            )
        elif end > n_agents * agent_width:
            expected_window = torch.cat(
                (field_2d[:, start:], field_2d[:, : end % (n_agents * agent_width)]),
                dim=1,
            )
        else:
...
def test_moving_window_3d(n_agents: int, agent_width: int, n_agents_per_window: int):
    y = 10

    field = torch.rand((n_agents * agent_width, y, n_agents * agent_width))
    field[: n_agents_per_window * agent_width, : n_agents_per_window * agent_width] = (
        1.0
    )
    ...
    expected_obs = field[
        : n_agents_per_window * agent_width, :, : n_agents_per_window * agent_width
    ]
